...
\* Last modified Wed Dec 13 23:26:29 GMT 2023 by samue
...
AnyFull(self) == \E otherThread \in 2..NThreads:
        \/ (1 + (((threads[1].endr + 1) % size) - 1)) = threads[otherThread].start
...
WriterWrite:
        if threads[1].full = FALSE then
           sent[(1 + (((threads[1].endr + 1) % size) - 1))] := [
                Reader |-> "not-read",
                Writer |-> "written"
            ];
            threads[1].endr := 1 + (((threads[1].endr + 1) % size) - 1);
...
ReaderNotEmpty:
            ...
            threads[self].start := 1 + (((threads[self].start + 1) % size) - 1);
...
\* BEGIN TRANSLATION (chksum(pcal) = "7fc672a4" /\ chksum(tla) = "a7d92086")
...
ReaderNotEmpty(self) == /\ pc[self] = "ReaderNotEmpty"
                        ...
                        /\ threads' = [threads EXCEPT ![self].start = 1 + (((threads[self].start + 1) % size) - 1)]
...
WriterWrite(self) == /\ pc[self] = "WriterWrite"
                     /\ IF threads[1].full = FALSE
                           THEN /\ sent' = [sent EXCEPT ![(1 + (((threads[1].endr + 1) % size) - 1))] =                                                    [
                                                                                                            Reader |-> "not-read",
                                                                                                            Writer |-> "written"
                                                                                                        ]]
                                /\ threads' = [threads EXCEPT ![1].endr = 1 + (((threads[1].endr + 1) % size) - 1)]
                           ELSE /\ TRUE
